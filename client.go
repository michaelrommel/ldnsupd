...
func (p *Provider) getDomain(ctx context.Context, zone string) ([]libdns.Record, error) {
	...
	if err != nil {
		return libRecords, fmt.Errorf("ldnsupd unable to lookup host: %w", err)
	}

	for _, ip := range ips {
		parsedIp, err := netip.ParseAddr(ip)
		if err != nil {
			return libRecords, fmt.Errorf("ldnsupd unable to parse IP address '%s': %w", ip, err)
		}
	...
	if err != nil {
		return libRecords, fmt.Errorf("ldnsupd unable to lookup TXT records for '%s': %w", fqdn, err)
	}
	for _, t := range txt {
		if t == "" {
			continue
...
func (p *Provider) doRequest(ctx context.Context, domain string, params map[string]string) ([]string, error) {
	...
	if len(mainDomain) == 0 {
		return nil, fmt.Errorf("ldnsupd unable to find the main domain for: %s", domain)
	}

	if len(p.APIToken) != 36 {
		return nil, fmt.Errorf("ldnsupd API token must be a 36 characters long UUID, got: '%s'", p.APIToken)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("ldnsupd unable to create request: %w", err)
	}

	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("ldnsupd unable to make request: %w", err)
	}
	defer resp.Body.Close()

	bodyBytes, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("ldnsupd unable to read response body: %w", err)
	}

	body := string(bodyBytes)
	bodyParts := strings.Split(body, "\n")
	if bodyParts[0] != responseOK {
		return nil, fmt.Errorf("ldnsupd request failed, expected (OK) but got (%s), url: [%s], body: %s", bodyParts[0], u.String(), body)
	}
